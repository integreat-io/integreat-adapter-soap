...
test('should get soap response', async (t) => {
  const got = sinon.stub().resolves({ body: soapResponse, statusCode: 200 })
  const request = {
    action: 'GET',
    endpoint: {
...
test('should use post when no data for 1.1', async (t) => {
  ...
  t.is(
    got.args[0][0],
    'http://api1.test/Economy/InvoiceOrder/V001/InvoiceService.asmx'
  )
...
test('should call got with soap 1.2 content-type', async (t) => {
  ...
  const expectedHeaders = {
    'Content-Type':
      'application/soap+xml;charset=utf-8;action="http://tempuri.org/HelloSimple"',
  }
...
test('should return error with response data', async (t) => {
  const got = sinon.stub().resolves({ body: soapResponse, statusCode: 500 })
  const request = {
    action: 'GET',
    endpoint: {
      uri: 'http://api1.test/Economy/InvoiceOrder/V001/InvoiceService.asmx',
  ...
  t.truthy(response)
  t.is(response.status, 'error', response.error)
  t.is(typeof response.error, 'string')
  t.deepEqual(response.data, soapResponse)
})

test.todo('should return error message from statusMessage')
