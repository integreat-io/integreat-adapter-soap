...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
  <soap:Header>
    <Action soap:mustUnderstand="1" xmlns="http://schemas.microsoft.com/ws/2005/05/addressing/none">http://tempuri.org/IBossNetService/ContainerFull</Action>
  </soap:Header>
  <soap:Body>
    <CardNew xmlns="http://example.com/webservices/">
      <customerkey>AD0B2E69-4640-FBF4-9CC7-0C713FDF3B12</customerkey>
...
test('should set object at path', async (t) => {
  ...
  const expectedAction =
    'http://example.com/webservices/GetPaymentMethodsResponse'
...
test('should override soapAction namespace', async (t) => {
  ...
  const expectedAction =
    'http://example.com/stupidOtherNamespace/GetPaymentMethodsResponse'
...
test('should override entire soapAction', async (t) => {
  const data = {
    PaymentMethod: [
      { Id: '1', Name: 'Cash' },
      { Id: '2', Name: 'Invoice' },
  ...
  const request = {
    ...
    endpoint: {
      ...
      namespace: 'http://example.com/webservices',
      soapAction:
        'http://example.com/stupidOtherNamespace/And_GetPaymentMethodsResponse',
    },
    params: {},
  }
  const expectedAction =
...
test('should handle different namespaces and soap versions', async (t) => {
  ...
  const expectedAction =
    'http://travelcompany.example.org/reservation/travel/itinerary'
...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
  <soap:Body>
    <empty p3:nil="true" xmlns:p3="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://example.com/webservices/"/>
  </soap:Body>
</soap:Envelope>`
...
test('should include Header element with additional default namespace', async (t) => {
  const data = [
    {
      'soap:Header': {
        '*addr:Action': {
...
test('should await promise in data', async (t) => {
  const data = {
    GetPaymentMethodsResponse: {
      GetPaymentMethodsResult: {
        PaymentMethod: [
          { Id: '1', Name: Promise.resolve('Cash') },
          { Id: '2', Name: 'Invoice' },
        ],
        DontInclude: Promise.reject(new Error('Could not transform')),
      },
    },
  }
  const endpoint = {
