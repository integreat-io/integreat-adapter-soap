...
const prepareArray = (elements, soap) =>
  elements.length === 1
...
const prepareObject = (soap) => (element) => {
  ...
  return Object.keys(element).reduce(
    (props, key) => prepareProp(element, props, key, soap),
    {}
  )
}
...
const getSoapAction = (
  ...
  return `${ensureTrailingSlash(
    soapActionNamespace || namespaces[prefix]
  )}${name}`
}

const stripDefaultNs = (ns) => (ns.startsWith('*') ? ns.split(':')[1] : ns)

const stripDefaultNamespaces = (element) =>
...
function prepareData(
  ...
  soapAction,
  soapActionNamespace,
  { prefix = 'soap', xsiPrefix = 'xsi', version = '1.1' }
) {
...
// Traverse data and await values from all promises
async function unpromise(data) {
  if (typeof data !== 'object' || data === null || data instanceof Date) {
    return data
  } else if (typeof data.then === 'function') {
    try {
      return await data // Await promise
    } catch (err) {
      return undefined // Disregard rejects
    }
  } else if (Array.isArray(data)) {
    const ret = []
    for (const value of data) {
      ret.push(await unpromise(value))
    }
    return ret
  } else {
    const ret = {}
    for (const key in data) {
      ret[key] = await unpromise(data[key])
    }
    return ret
  }
}
...
async function serialize(request) {
  const {
    requestPath,
    namespace,
    soapAction: soapActionOption,
    soapActionNamespace,
    soap = {},
  } = request.endpoint
  const { soapAction, ...preparedObj } = prepareData(
    await unpromise(request.data),
    requestPath,
    namespace,
    soapActionOption,
    soapActionNamespace,
  ...
  const options = {
    compact: true,
    spaces: soap.spaces || 2,
    textFn: encodeText,
    allowDoubleSanitization: true,
  }
