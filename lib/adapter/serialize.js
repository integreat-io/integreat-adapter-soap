...
const prepareData = (data, path, namespace, { prefix = 'soap', version = '1.1' }) => {
  ...
  const element = dotProp.set({}, fullPath, data)
...
async function serialize (request) {
  ...
  const preparedObj = prepareData(request.data, requestPath, namespace, soap)

  const data = convert.js2xml(preparedObj, { compact: true, spaces: soap.spaces || 2 })
