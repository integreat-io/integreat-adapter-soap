...
const getSoapAction = (data, soapActionNamespace, namespaces, soapPrefix) => {
...
const createXmlObject = (element, namespaces, data, soapActionNamespace, prefix, xsiPrefix) =>
  stripDefaultNamespaces({
    soapAction: (data) ? getSoapAction(element, soapActionNamespace, namespaces, prefix) : undefined,
    _declaration: { _attributes: { version: '1.0', encoding: 'utf-8' } },
    ...declareNamespaces(prepareObject({ xsiPrefix })(element), namespaces)
  })

const prepareData = (data, path, namespace, soapActionNamespace, { prefix = 'soap', xsiPrefix = 'xsi', version = '1.1' }) => {
  ...
  return createXmlObject(element, namespaces, data, soapActionNamespace, prefix, xsiPrefix)
}
...
async function serialize (request) {
  const { requestPath, namespace, soapActionNamespace, soap = {} } = request.endpoint
  const { soapAction, ...preparedObj } = prepareData(request.data, requestPath, namespace, soapActionNamespace, soap)
