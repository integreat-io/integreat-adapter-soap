...
test('should return nil as null with custom xsi namescape', async (t) => {
  const response = {
    status: 'ok',
    data: soapResponseWithNull.replace('xsi:', 'i:'),
  }
  const endpoint = {
    uri: 'http://api1.test/Economy/InvoiceOrder/V001/InvoiceService.asmx',
    soap: { xsiPrefix: 'i' },
...
test('should normalize data on error too', async (t) => {
  const response = { status: 'error', data: soapResponse }
  const endpoint = {
    uri: 'http://api1.test/Economy/InvoiceOrder/V001/InvoiceService.asmx',
  }
  const request = {
