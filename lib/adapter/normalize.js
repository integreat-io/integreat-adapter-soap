...
const normalizeElement = (element, defaultPrefix, xsiPrefix) => {
  ...
  if (keys.length === 0) {
    ...
    return normalizeObject(element, defaultPrefix, xsiPrefix)
  }
}

const shouldNormalizeKey = (prefix, xsiPrefix) => (key) => !(prefix === '@' && key === `${xsiPrefix}:nil`)

const setProp = (key, value, defaultPrefix, xsiPrefix) => (key === '_attributes')
  ? normalizeObject(value, defaultPrefix, xsiPrefix, '@')
  : (shouldSkip(key)) ? null : { [key]: normalizeElement(value, defaultPrefix, xsiPrefix) }

const normalizeObject = (obj, defaultPrefix, xsiPrefix, prefix = '') => {
  if (Array.isArray(obj)) {
    return obj.map((o) => normalizeObject(o, defaultPrefix, xsiPrefix, prefix))
  }
  ...
  return Object.keys(obj)
    .filter(shouldNormalizeKey(prefix, xsiPrefix))
    .map(key => setProp(prefix + stripPrefix(key, defaultPrefix), obj[key], defaultPrefix, xsiPrefix))
