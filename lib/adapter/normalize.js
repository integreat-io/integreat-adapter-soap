...
const getPrefixFromData = (data) => {
  const envKey = Object.keys(data).find((key) => envRegex.test(key))
  const match = envRegex.exec(envKey)
...
const getDataAtPath = (data, path) => {
  const prefix = getPrefixFromData(data)
  ...
  const dataObj = dotProp.get(body, path)
  if (header) {
