const convert = require('@kjellmorten/xml-js')
...
const shouldNormalizeKey = (prefix, xsiPrefix) => (key) =>
  !(prefix === '@' && key === `${xsiPrefix}:nil`)

const setProp = (key, value, defaultPrefix, xsiPrefix) =>
  key === '_attributes'
    ? normalizeObject(value, defaultPrefix, xsiPrefix, '@')
    : shouldSkip(key)
    ? null
    : { [key]: normalizeElement(value, defaultPrefix, xsiPrefix) }
...
const normalizeObject = (obj, defaultPrefix, xsiPrefix, prefix = '') => {
  ...
  return Object.keys(obj)
    ...
    .map((key) =>
      setProp(
        prefix + stripPrefix(key, defaultPrefix),
        obj[key],
        defaultPrefix,
        xsiPrefix
      )
